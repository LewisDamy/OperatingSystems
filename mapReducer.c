...
/*  C PROGRAM TO IMPLEMENT MAP-REDUCE
  Grupo:
...
int writeNewFile(void)
{
  char *fileName = "saida.txt"; // cria nome do arquivo

  FILE *fp = fopen(fileName, "w"); // abre arquivo em modo "write"
  NodeTypeHash pAux;               // cria tipo no pAux para fazer iteracao nos elementos de cada indice da tabela hash

  if (fp == NULL)
  { // se fp for null, deu erro
    printf("Erro ao abrir arquivo. %s \n", fileName);
    return -1;
  }
  for (int i = 0; i < 5; i++)
  { // faz a iteracao de cada indice da tabela hash
    for (pAux = TH[i].primeiro; pAux != NULL; pAux = pAux->prox)
    {                                                     // faz a iteracao de cada elemento do indice correspondente
      fprintf(fp, "%i %s,\n", pAux->qtWord, pAux->chave); // escreve o numero de repeticoes e a palavra na linha do arquivo
    }
  }
  return 0; // retorno 0 caso nao haja nenhum erro
}
