...
{                                                 // remove da tabela hash
  return removeElementoLE(&(Taux[index]), value); // função chamada para remover o elemento na posição específica da chave
}
...
{ // imprime a tabela hash
  for (int i = 0; i < tamHash; i++)
  {
    ...
    imprimeElementosLE(&Taux[i]); // chama a função para imprimir os outros elementos encadeados
    printf("\n");
  }
}
